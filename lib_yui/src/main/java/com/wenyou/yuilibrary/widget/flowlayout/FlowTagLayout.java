...
public class FlowTagLayout extends ViewGroup {
    ...
    private void initAttrs(Context context, AttributeSet attrs) {
        ...
        if (entriesID != 0) {
            BaseTagAdapter tagAdapter = setItems(getResources().getStringArray(entriesID));
            int selectedIDs = typedArray.getResourceId(R.styleable.FlowTagLayout_ftl_selected, 0);
            if (selectedIDs != 0) {
                tagAdapter.setSelectedPositions(getResources().getIntArray(selectedIDs));
            }
        }
        typedArray.recycle();
    }
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        int childLeft = getPaddingLeftStart();
        ...
        for (int i = 0, childCount = getChildCount(); i < childCount; i++) {
            ...
            if (childLeft + realChildWidth > flowWidth - getPaddingLeft() - getPaddingRight()) {
                ...
                childLeft = getPaddingLeftStart();
            }
            //布局
            if (isRtl()) {
                int end = flowWidth - (childLeft + getMarginLeftStart(lp));
                int top = childTop + lp.topMargin;
                int start = end - childWidth;
                int bottom = top + childHeight;
                childView.layout(start, top, end, bottom);
            } else {
                int left = childLeft + getMarginLeftStart(lp);
                int top = childTop + lp.topMargin;
    ...
    private void reloadData() {
        ...
        for (int index = 0; index < mAdapter.getCount(); index++) {
            ...
            final View childView = mAdapter.getView(index, null, this);
            addView(childView, mMarginLayoutParams);

            if (mAdapter != null) {
                boolean isSelected = mAdapter.isSelectedPosition(index);
                //判断一下模式
                if (mTagCheckMode == FLOW_TAG_CHECKED_SINGLE) {
                    //单选只有第一个起作用
                    if (isSelected && !oneSelected) {
    ...
    private FlowTagLayout setSelectedIndexs(List<Integer> selectedIndexs) {
        if (mAdapter != null) {
            mAdapter.setSelectedIndex(selectedIndexs);
        }
        return this;
    }
