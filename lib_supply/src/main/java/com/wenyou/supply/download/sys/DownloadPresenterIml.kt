...
import com.wenyou.supply.utils.SupplyLogUtils
import java.io.File
 ...
 * @author: jy
 */
class DownloadPresenterIml(
    private val context: Context,
    private val downloadManager: DownloadManager,
    private val downloadView: DownloadView
) :
    DownloadPresenter {
    ...
    companion object DownHandler : Handler() {
        ...
        override fun handleMessage(msg: Message) {
            ...
            if (msg.arg1 >= 0 && msg.arg2 > 0) {
                SupplyLogUtils.i(
                    "DownloadManager",
                    "current progress is ${msg.arg1 / msg.arg2.toFloat()}"
                )
            }
        }
    }
...
//                setNotificationVisibility(DownloadManager.Request.VISIBILITY_HIDDEN)
            ...
            if (isSavePoint) {
                val downloadId = getDownloadId(url)
                if (downloadId != 0L) {
                    when (getDownloadStatus(downloadId)) {
                        ...
                        DownloadManager.STATUS_RUNNING -> SupplyLogUtils.e(
                            "DownloadManager",
                            "download is running"
                        )
    ...
    private fun removeAndReDownloadFile(
        downloadId: Long,
        index: Int,
    ...
    private fun addDownloadEnqueueAndSaveId(
        index: Int,
        request: DownloadManager.Request,
        url: String
    ) {
        downloadIds[index] = downloadManager.enqueue(request)
        SupplyLogUtils.i("DownloadManager", "put download id", "$url :: ${downloadIds[index]}")
        saveDownloadId(url, downloadIds[index]!!)
    }
    ...
    private inner class DownloadChangeObserver(val downloadIds: Array<Long?>) :
    ...
    private fun getDownloadId(url: String): Long {
        val sp =
            context.applicationContext.getSharedPreferences("configDownload", Context.MODE_PRIVATE)
        return sp.getLong(url, 0)
    }

    private fun saveDownloadId(url: String, value: Long) {
        val spEditor =
            context.applicationContext.getSharedPreferences("configDownload", Context.MODE_PRIVATE)
                .edit()
        spEditor.putLong(url, value)
        spEditor.apply()
    }
