...
public class RomUtils {
    ...
    private static boolean check(String rom) {
        ...
        if (null != tmpName && !tmpName.isEmpty()) {
            sName = tmpName;
            sVersion = tmpVersion;
            YLogUtils.INSTANCE.iTag(TAG, "sp--name：" + sName + "--version：" + sVersion + "---rom：" + rom);
            return sName.equals(rom);
        }
        ...
        SharedPreferencesConfigUtils.getInstance().putApply(SharedPreferencesConfigUtils.ROM, sName);
        SharedPreferencesConfigUtils.getInstance().putApply(SharedPreferencesConfigUtils.ROM_VERSION, sVersion);

        YLogUtils.INSTANCE.iTag(TAG, "getProp--name：" + sName + "--version：" + sVersion + "---rom：" + rom);
        return sName.equals(rom);
    }
