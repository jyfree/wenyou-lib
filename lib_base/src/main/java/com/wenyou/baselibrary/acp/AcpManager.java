...
public class AcpManager {
    ...
    public void checkRequestPermissionRationale(Activity activity) {
        ...
        if (isShowRational) {
            rationale = AcpConstant.deniedPermissionList.size() > 0;
        }
    ...
    private void showRationalDialog(final Activity activity, final String[] permissions) {
        ...
        if (AcpApply.Companion.get().isCustomApply()) {
            AcpApply.Companion.get().showApplyDialog(new AcpApplyActionListener() {
                @Override
                public String getRationalMessage() {
                    return permissionOptions.getRationalMessage();
                }
